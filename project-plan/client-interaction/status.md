...
- **Family Case Consolidation Complete (2026-09-23)**: Merged the Family Case pages into Session Notes (`/admin/sessions`), making Session Notes the single consolidated place for a family case. Retired and deleted `HouseholdDossier.tsx` and `AdminFamilies.tsx`. Kept `/admin/families` and `/admin/families/:householdId` as redirects. `FamilyGlancePanel` handles family case field updates and adding members. `MemberStudyModal` handles member editing and removal with confirmation. Added "+ New session" button supporting blank sessions and booking conversion. Added "Start family case" modal across CRM, Client Dossier, and Session Notes.
