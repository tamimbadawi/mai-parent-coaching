-- Migration: Add engagement preferences to profiles and create customer_journey_state view
-- Date: 2026-09-21

-- 1. Add engagement columns to profiles table
alter table public.profiles
  add column if not exists engagement_status text not null default 'active'
    check (engagement_status in ('active', 'paused', 'opted_out')),
  add column if not exists engagement_cadence_days integer not null default 14
    check (engagement_cadence_days >= 7);

-- Index for CRM filtering
create index if not exists idx_profiles_engagement_status
  on public.profiles (engagement_status);

-- 2. Create the computed customer_journey_state view
-- Derives track, lifecycle stage, session history, and engagement recency dynamically.
...
base_journey as (
  ...
  where p.role = 'student' -- only clients/students, exclude admins
)
select
  bj.client_id,
  bj.parent_name,
